...
export function ParcelManagement({ currentRole, currentUser }: ParcelManagementProps) {
  ...
  const filteredParcels = transporterScopedParcels.filter((p) => {
    ...
    return matchesSearch && matchesSenderSearch && matchesRecipientSearch && matchesStatus;
  });
    ...
    <div className="space-y-6">
      ...
      >
        {availableFilters.slice(1).map((filter) => {
          const count = transporterScopedParcels.filter((p) => p.status === filter.value).length;
          return (
        ...
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto border-border bg-card">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-foreground">
              ...
              {selectedParcel && (
                <CopyTrackingNumberButton trackingNumber={selectedParcel.trackingNumber} className="h-8 w-8" />
              )}
            </DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={handleCreateDialogChange}>
        <DialogContent className="max-h-[92vh] max-w-6xl overflow-hidden border-border bg-card p-0">
          <DialogHeader>
            <div className="border-b border-border px-6 py-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  <DialogTitle className="text-foreground">
                    Enregistrer un colis au point de collecte
                  </DialogTitle>
                  <DialogDescription className="mt-1">
                    Le colis sera cree directement avec le statut de prise en charge au point de
                    collecte.
                  </DialogDescription>
                </div>
          ...
          <div className="min-h-0">
            <ScrollArea className="h-[calc(92vh-176px)]">
              <div className="grid gap-6 px-6 py-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.85fr)]">
                  ...
                  <section className="space-y-4 rounded-2xl border border-border p-5">
                    ...
                    <div className="min-w-0">
                      ...
                      <Textarea
                        ...
                        className="min-h-[108px] resize-none bg-secondary"
                      />
                    </div>
                  </section>
                </div>
                ...
                <aside className="min-w-0 space-y-6">
                  ...
                  <section className="space-y-4 rounded-2xl border border-border p-5">
                    ...
                    <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-secondary/40 px-4 py-5 text-center text-sm text-muted-foreground transition-colors hover:border-primary/50 hover:text-foreground">
